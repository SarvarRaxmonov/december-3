...
async def  botquery(callback_query_data: types.CallbackQuery):
        ...
        if check_sub_channel(await bot.get_chat_member(chat_id=CHANNEL_ID, user_id=callback_query_data.from_user.id)) & check_sub_channel(await bot.get_chat_member(chat_id=CHANNEL_ID2, user_id=callback_query_data.from_user.id)):
                ...
                qorisura = {'1':{'r':nav.sura5, 'qori':'Mishary bin Rashed Alafasy'},'2':{'r':nav.sura9, 'qori':'Abdul Rahman Al-Sudais'},'3':{'r':nav.sura13, 'qori':' Saud Al-Shuraim '}, '4':{'r':nav.sura17, 'qori':'Saad Al-Ghamidi'},'5':{'r':nav, 'qori':'Bandar Bin Abdulaziz Baleela'},'6':{'r':nav, 'qori':' Idris Akbar '},
                            '7':{'r':nav, 'qori':'Yasser Al-Dosari'}, '8': {'r':nav.sura1,'qori':'Maher Al-Mueaqly'}, '9':{'r':nav, 'qori':' 9. Abdullah Awad Al-Juhany'}, '10':{'r':nav, 'qori':'Abdul Basit Abdessamad '},'11':{'r':nav, 'qori':'Zaki Daghistani'},'12':{'r':nav, 'qori':'Sahl Yassin '},'13':{'r':nav, 'qori':'Mohammed Ayyub'},'14':{'r':nav, 'qori':' Raad Al-Kurdi'},'15':{'r':nav,'qori':' Abdulrashed Soufi'}}                            
                ...
                for vare , re in jsuz.hmuchun.items():
                         if  callback_query_data.data == f'{vare}':
                             ...
                             for i in range(n,v):
                               if i > 150:
                                      await bot.send_audio(callback_query_data.from_user.id, audio=f'https://t.me/MisharyRoshidQuran/{i}')
                               if i < 200:
                                       await bot.send_audio(callback_query_data.from_user.id, audio=f'https://t.me/Sheikh_Abdul_Rahman_Al_Sudais/{i}')
